...
hardware-api/
├── gps.php                   # GPS telemetry endpoint (updates vehicle lat/lng/speed)
├── passenger.php             # Passenger counter endpoint (vision AI people counting)
└── test.php                  # Web-based hardware simulator for testing both endpoints
```
...
| **OpenRouter**          | openrouter.ai                                           | Vision AI (Gemini 2.0 Flash) for passenger counting        |
...
### 6. Hardware Device Updates Vehicle Data

```
GPS module on bus sends position fix every ~3 seconds
    │
    ▼
POST hardware-api/gps.php with {data: {bus_id, latitude, longitude, speed, direction}}
    │
    ▼
Validates bus_id, lat/lng ranges
    │
    ▼
Updates vehicle in vehicles.json (lat, lng, speed, bearing) with LOCK_EX
    │
    ▼
200 OK → {success, vehicle_id, updated: {lat, lng, speed}}
```

```
Onboard camera captures frame
    │
    ▼
POST hardware-api/passenger.php with vehicle_id + image (multipart)
    │
    ▼
Validates vehicle_id, image type (JPEG/PNG/WebP), size (≤10MB)
    │
    ▼
Image base64-encoded → sent to OpenRouter (Gemini 2.0 Flash) vision API
    │
    ▼
AI counts visible people → returns {count, confidence}
    │
    ▼
Updates vehicle's passengers field in vehicles.json with LOCK_EX
    │
    ▼
200 OK → {success, vehicle_id, passengers, confidence}
```
...
// vehicles.json
[
  {"id": 1, "name": "Bus #101", "lat": 27.71, "lng": 85.32, "routeId": 1, "speed": 28, "passengers": 11, "capacity": 40, "moving": true, "bearing": 90, "color": "#1d4ed8"}
]
