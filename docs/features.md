...
- [x] Passenger count and capacity displayed on vehicle assignment cards (e.g., 11/40)
- [x] Passenger count and capacity shown in vehicle map popups
...
- [x] Create vehicles (name, lat, lng, routeId, speed, passengers, capacity, color, icon, image)
- [x] Assign vehicles to routes (dropdown)
- [x] Edit passenger count and capacity in inspector
...
### GPS Telemetry Endpoint (`hardware-api/gps.php`)

- [x] POST endpoint receiving JSON payload from GPS hardware modules
- [x] Validates bus_id (required, numeric), latitude (-90 to 90), longitude (-180 to 180)
- [x] Updates vehicle position (lat, lng, speed, bearing) in vehicles.json
- [x] File locking (LOCK_EX) for concurrent write safety
- [x] Returns updated fields on success

### Passenger Counter Endpoint (`hardware-api/passenger.php`)

- [x] POST endpoint receiving multipart form data (vehicle_id + image)
- [x] Validates image type (JPEG, PNG, WebP) and size (max 10 MB)
- [x] Sends image to OpenRouter vision AI (Gemini 2.0 Flash) for people counting
- [x] AI returns count and confidence level (high/medium/low)
- [x] Updates vehicle's passengers field in vehicles.json
- [x] Handles partial bodies, occlusion, and empty frames

### Hardware Simulator (`hardware-api/test.php`)

- [x] Web-based IoT console with dark theme
- [x] GPS module: vehicle selector, lat/lng/speed/direction inputs, send button
- [x] GPS auto-simulate mode (3s interval with random drift)
- [x] GPS randomize (random Kathmandu valley coordinates)
- [x] Passenger camera: vehicle selector, image drag-and-drop zone, count & update button
- [x] Console log panel with timestamped colored entries (gps/cam/err/info)
- [x] Loads vehicle list from vehicles.json for dropdowns
