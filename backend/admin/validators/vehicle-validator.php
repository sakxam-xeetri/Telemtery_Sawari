...
class VehicleValidator
{
    public static function validate(array $data, bool $isUpdate = false): array
    {
        ...
        if (isset($data['passengers'])) {
            if (intval($data['passengers']) < 0)
                $errors[] = 'Passenger count cannot be negative';
        }

        if (isset($data['capacity'])) {
            if (intval($data['capacity']) < 1)
    ...
    public static function defaults(array $data): array
    {
        ...
        $data['passengers'] = intval($data['passengers'] ?? 0);
        $data['capacity'] = intval($data['capacity'] ?? 40);
